...
def fetch_and_store_corporate_actions(limit=None, batch_size=50):
    ...
    count = 0
    new_actions = 0
    logger.info(f"Fetching historical corporate actions for {total} companies" + (f" (limited to {limit})" if limit else ""))
    print(f"Fetching historical corporate actions for {total} companies" + (f" (limited to {limit})" if limit else ""))
    for i in range(0, len(companies), batch_size):
        ...
        print(f"Processed {count}/{total} companies. Added {len(new_actions_batch)} new actions.")
    session.close()
