...
# Set default values
if (length(args) == 0) {
  log_message("Enter input file with path as argument during runtime...")
...
log_message(sprintf("Loading momentum signals data from: %s", data_file))
if (!file.exists(data_file)) {
  stop("Momentum signals file not found: ", data_file)
}

dt <- fread(data_file)
log_message(sprintf("Loaded %d rows from %s", nrow(dt), basename(data_file)))
...
# Save results
# Generate output directory based on input file name
dataset_type <- tolower(sub("momentum_signals_([a-z]+).*", "\\1", basename(data_file)))
output_dir <- file.path(dirname(data_file), "trade_analysis", dataset_type)
...
output_file <- file.path(output_dir, "trades.csv")
...
generate_trade_summary <- function(dt) {
  ...
  if (nrow(trades) > 0) {
    ...
    # Calculate max drawdown based on cumulative P&L
    trades[, drawdown := {
      if (.N > 0) {
        running_max <- cummax(PercCumulativePL)
        drawdowns <- ifelse(running_max > 0, (running_max - PercCumulativePL) / abs(running_max) * 100, 0)
        max(drawdowns, na.rm = TRUE)
      } else {
        0
      }
    }, by = company_id]
    max_drawdown <- if (all(is.na(trades$drawdown))) 0 else max(trades$drawdown, na.rm = TRUE)
    ...
    if (max_drawdown > 0) {
      total_return <- sum(trades$pnl_pct, na.rm = TRUE)
...
if (nrow(trade_summary) > 0) {
  ...
  # Save both trade details and performance summary to the output directory
  # 1. Save detailed trades with company name
  trade_details_file <- file.path(output_dir, "trade_details.csv")
  ...
  metrics_file <- file.path(output_dir, "performance_metrics.csv")
