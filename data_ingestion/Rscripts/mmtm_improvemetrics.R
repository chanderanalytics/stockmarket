   ...
   dt[company_id %in% current_companies, {
     ...
     tryCatch({
       ...
       if (sum(valid_returns) >= 21) {
        ...
        min_window <- 21  # Minimum required days for meaningful drawdown calculation
        max_window <- 252  # Maximum lookback period
        
        if (.N >= min_window) {
          # Use the smaller of available days or max_window, but at least min_window
          window_size <- min(.N, max_window)
          
          # Calculate cumulative maximum (peak) price
          roll_max <- frollapply(close, window_size, max, align = "right", fill = NA)
          ...
          vol_21d <- frollapply(returns, 21, sd, align = "right", fill = NA) * sqrt(252)
          
          # Get the minimum drawdown (which is the maximum drawdown)
          max_dd <- frollapply(drawdown, window_size, min, align = "right", fill = NA)
          
          # Normalize drawdown by volatility (with a minimum volatility floor of 5%)
          vol_normalized_dd <- max_dd / pmax(vol_21d, 0.05)
          
          # Cap the normalized drawdown at 5x volatility
          max_dd <- pmin(vol_normalized_dd, 5)
          
          # Ensure we have enough non-NA values
          if (sum(!is.na(max_dd)) < min_window) {
            max_dd <- rep(NA_real_, .N)
          }
        } else {
...
dt[, risk_score := {
  ...
  # 1.1 Volatility Risk (21-day annualized)
  vol_risk <- safe_ecdf(vol_21d)  # Higher vol = higher risk

  # 1.2 Max Drawdown Risk (normalized by ATR)
  atr_risk <- if (all(is.na(atr)) || all(atr == 0, na.rm = TRUE)) {
    rep(NA_real_, .N)
  } else {
    normalized_dd <- abs(max_drawdown_252d) / (atr / close)  # Drawdown in ATR terms
    safe_ecdf(normalized_dd)
  }

  # 1.3 Value at Risk (1-day 95% VaR)
  var_risk <- safe_ecdf(abs(var_1d))  # Higher VaR = higher risk

  # 1.4 Position Size Risk (based on ATR and account size)
  position_risk <- if (all(is.na(atr)) || all(atr == 0, na.rm = TRUE)) {
    rep(NA_real_, .N)
  } else {
    # Calculate position size as % of account at 1% risk per trade
    # Higher position size = higher risk
    position_size_pct <- (atr * 100) / close  # Position size as % of price
    safe_ecdf(position_size_pct)
  }

  # 1.5 Volume Stability (21-day CV of volume)
  vol_sd <- frollapply(volume, 21, sd, na.rm = TRUE, align = "right")
  vol_mean <- frollmean(volume, 21, na.rm = TRUE, align = "right")
  vol_cv <- ifelse(vol_mean > 0, vol_sd / vol_mean, 0)
  volume_stability <- safe_ecdf(vol_cv * 100)  # Convert to percentage for ECDF

  # 1.6 Price Stability (21-day ATR as % of price)
  atr_pct <- ifelse(close > 0, (atr / close) * 100, 0)
  price_stability <- safe_ecdf(atr_pct)  # Higher ATR% = higher risk

  # 2. Define component weights
  weights <- c(
    vol_risk = 0.25,          # 25% - Overall market risk
    atr_risk = 0.25,          # 25% - Normalized drawdown risk
    var_risk = 0.20,          # 20% - Tail risk
    position_risk = 0.15,     # 15% - Position sizing risk
    volume_stability = 0.10,  # 10% - Liquidity risk
    price_stability = 0.05    # 5%  - Price movement risk
  )
  ...
  for (i in 1:.N) {
   ...
   if (length(row_values) >= 3) {  # Require at least 3 valid components
    # Calculate weighted average, normalizing by sum of available weights
    risk_score[i] <- sum(row_values * row_weights) / sum(row_weights)
    
    # Apply non-linear scaling to better separate risk levels
    # This will make the distribution more balanced across the 0-100 range
    risk_score[i] <- 100 * (1 - exp(-risk_score[i] / 33.33))  # Exponential scaling
   }
  } }
   ...
   # Apply some jitter to break ties in scores
   jitter_amount <- runif(length(risk_score), -0.5, 0.5)
   risk_score <- risk_score + jitter_amount
 ...
 # w) Risk Categories - Adjusted for better distribution
# First, calculate percentiles of risk_score for better distribution
quantiles <- quantile(dt$risk_score, probs = seq(0, 1, 0.2), na.rm = TRUE, names = FALSE)

# Ensure we have exactly 5 breaks (6 values including min and max)
if (length(unique(quantiles)) < 6) {
  # Fallback if not enough unique quantiles
  breaks <- seq(0, 100, 20)
} else {
  breaks <- quantiles
  breaks[1] <- 0  # Ensure min is 0
  breaks[length(breaks)] <- 100  # Ensure max is 100
}

# Apply the breaks to create risk categories
dt[, risk_category := cut(risk_score,
            breaks = breaks,
            labels = c("Very High", "High", "Medium", "Low", "Very Low"),
            include.lowest = TRUE)]

# Log the distribution of risk categories
risk_dist <- dt[!is.na(risk_category), .(count = .N), by = risk_category]
log_message("Risk Category Distribution:")
for (i in 1:nrow(risk_dist)) {
  log_message(sprintf(" - %s: %d (%.1f%%)", 
           as.character(risk_dist$risk_category[i]), 
           risk_dist$count[i],
           risk_dist$count[i] / nrow(dt[!is.na(risk_category)]) * 100))
}
...
main <- function() {
 ...
 # Parse arguments
 if (length(args) > 0) {
   # First argument is reference date
   ref_date <- tryCatch({
     as.Date(args[1])
   }, error = function(e) {
   ...
   # Second argument is max number of companies (optional)
   if (length(args) > 1) {
     max_companies <- as.integer(args[2])
     if (is.na(max_companies) || max_companies <= 0) {
       log_message(sprintf("Invalid max_companies: %s. Processing all companies.", args[2]), "WARN")
       max_companies <- NULL
     } else {
       log_message(sprintf("Limiting to %d companies", max_companies))
     }
   }
 }
 ...
 if (!is.null(con)) {
  tryCatch({
   ...
   if (!is.null(max_companies)) {
     query <- paste0(query, sprintf(" LIMIT %d", max_companies))
   }
