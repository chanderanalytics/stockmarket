...
# Argument parsing: ref_date, scenario, limit_companies
parse_arguments <- function() {
  args <- commandArgs(trailingOnly = TRUE)

  if (any(args %in% c("--help", "-h"))) {
    cat(paste0(
      "Usage: Rscript data_ingestion/Rscripts/s4_mmtm_clean_trade_tracker_fixed.R <YYYY-MM-DD> [--scenario=momentum_0] [--limit_companies=N] [--recompute_trades]\n\n",
      "Environment defaults (optional):\n",
      "  MMTM_SCENARIO=momentum_0\n",
      "  MMTM_LIMIT_COMPANIES=100\n",
      "  MMTM_RECOMPUTE_TRADES=true|false\n"
    ))
  ...
  env_recompute <- Sys.getenv("MMTM_RECOMPUTE_TRADES")
  env_next_day <- Sys.getenv("MMTM_EXECUTE_NEXT_DAY")
  scenario_default <- if (nzchar(env_scenario)) env_scenario else "momentum_0"
  limit_default <- if (!is.na(env_limit) && env_limit > 0) env_limit else NULL
  recompute_default <- if (nzchar(env_recompute)) is_truthy(env_recompute) else FALSE
  ...
  params <- list(
    ...
    recompute_trades = recompute_default,
    execute_next_day = next_day_default
  )

  if (length(args) > 0 && !startsWith(args[1], "--")) {
    params$ref_date <- tryCatch(as.Date(args[1]), error = function(e) Sys.Date())
  }
  if (length(args) > 0) {
    ...
    if (length(limit_arg) == 1) {
      limit_val <- suppressWarnings(as.integer(sub("^--limit_companies=", "", limit_arg)))
      if (!is.na(limit_val) && limit_val > 0) params$limit_companies <- limit_val
    }

    if (any(grepl("^--recompute_trades$", args))) {
      params$recompute_trades <- TRUE
    }
...
main <- function() {
  ...
  log_message(sprintf("Loading momentum signals data (ref_date=%s, scenario=%s)...", params$ref_date, params$scenario))

# Load companies fundamental data from database
companies_data <- NULL
tryCatch({
  ...
  log_message("Loading companies fundamental data from database...")
  companies_data <- as.data.table(dbGetQuery(con, 
  "SELECT company_id, name, market_capitalization, 
  sales, operating_profit, profit_after_tax,
  cap_class, cap_class_sebi
  FROM merged_price_baseline_probabilities_wide"))
  log_message(sprintf("Loaded %d companies with fundamental data", nrow(companies_data)))
  
  # Create fundamental buckets based on PowerBI logic
  # Use existing market cap classification from database
  companies_data[, mcap_bucket := cap_class_sebi]
  
  companies_data[, sales_bucket := fifelse(
    sales <= 0, "0:Unknown",
    fifelse(sales < 90, "1:<$1M",
    fifelse(sales < 900, "2:$1M-$10M",
    fifelse(sales < 4500, "3:$10M-$50M",
    fifelse(sales < 9000, "4:$50M-$100M",
    fifelse(sales < 45000, "5:$100M-$500M",
    fifelse(sales < 90000, "6:$500M-$1B", "7:>$1B"
  )))))))]
  
  companies_data[, op_profit_bucket := fifelse(
    operating_profit == 0, "0:Unknown",
    fifelse(operating_profit < 90, "1:<$1M",
    fifelse(operating_profit < 900, "2:$1M-$10M",
    fifelse(operating_profit < 4500, "3:$10M-$50M",
    fifelse(operating_profit < 9000, "4:$50M-$100M",
    fifelse(operating_profit < 45000, "5:$100M-$500M",
    fifelse(operating_profit < 90000, "6:$500M-$1B", "7:>$1B"
  )))))))]
  
  companies_data[, pat_bucket := fifelse(
    profit_after_tax == 0, "0:Unknown",
    fifelse(profit_after_tax < 90, "1:<$1M",
    fifelse(profit_after_tax < 900, "2:$1M-$10M",
    fifelse(profit_after_tax < 4500, "3:$10M-$50M",
    fifelse(profit_after_tax < 9000, "4:$50M-$100M",
    fifelse(profit_after_tax < 45000, "5:$100M-$500M",
    fifelse(profit_after_tax < 90000, "6:$500M-$1B", "7:>$1B"
  )))))))]
  
  dbDisconnect(con)
  log_message("Fundamental buckets created successfully")
  
}, error = function(e) {
  log_message(sprintf("Failed to load fundamental data: %s", e$message), "WARN")
  companies_data <- NULL
})

# Prefer s3 scenario output; fallback to legacy full file if present
scenario_data_file <- sprintf("output/mmtm/%s_%s.csv", params$scenario, format(params$ref_date, "%Y-%m-%d"))
legacy_data_file <- "output/momentum_cycle_signals_full.csv"

data_file <- if (file.exists(scenario_data_file)) {
  scenario_data_file
} else if (file.exists(legacy_data_file)) {
  legacy_data_file
} else {
  stop("Momentum signals file not found. Expected either: ", scenario_data_file, " or ", legacy_data_file)
}
...
# Ensure entry/exit date columns exist as character to avoid type conflicts
if (!"entry_date" %in% names(dt)) {
  dt[, entry_date := NA_character_]
} else {
  dt[, entry_date := as.character(entry_date)]
}
if (!"exit_date" %in% names(dt)) {
  dt[, exit_date := NA_character_]
} else {
  dt[, exit_date := as.character(exit_date)]
}
data.table::setorder(dt, company_id, date)

if (!is.null(params$limit_companies)) {
  log_message(sprintf("Limiting to first %d companies...", params$limit_companies))
  selected_companies <- unique(dt$company_id)[1:min(params$limit_companies, length(unique(dt$company_id)))]
  dt <- dt[company_id %in% selected_companies]
  log_message(sprintf("After limit: %d rows for %d companies", nrow(dt), length(selected_companies)))
}

# Function to process trades from signals
process_trades_from_signals <- function(dt, execute_next_day = FALSE) {
    ...
    for (i in 1:nrow(comp_data)) {
      ...
      if (!in_trade && !is.na(current_status) && startsWith(current_status, "ENTRY_")) {
        if (isTRUE(execute_next_day)) {
          # Set pending entry for next day execution
          pending_entry <- TRUE
          log_message(sprintf("  [%s] PENDING TRADE ENTRY for next day", comp_data$date[i]))
        } else {
          # Execute immediately
          in_trade <- TRUE
      ...
      # Execute pending entry on next day
      if (isTRUE(execute_next_day) && isTRUE(pending_entry) && !in_trade) {
        in_trade <- TRUE
        entry_px <- comp_data$open[i]
        entry_dt <- as.character(comp_data$date[i])
        data.table::set(comp_data, i, "entry_price", entry_px)
        data.table::set(comp_data, i, "entry_date", entry_dt)
        data.table::set(comp_data, i, "trade_active", TRUE)
        ...
        log_message(sprintf("  [%s] NEW TRADE ENTRY at %.2f (next day open)", 
                           comp_data$date[i], entry_px))
      }
      ...
      if (in_trade) {
        ...
        if (!is.na(current_status) && startsWith(current_status, "EXIT_")) {
          if (isTRUE(execute_next_day)) {
            # Set pending exit for next day execution
            pending_exit <- TRUE
            log_message(sprintf("  [%s] PENDING TRADE EXIT for next day", comp_data$date[i]))
          } else {
            # Execute immediately
            in_trade <- FALSE
      ...
      # Execute pending exit on next day
      if (isTRUE(execute_next_day) && isTRUE(pending_exit) && in_trade) {
        in_trade <- FALSE
        exit_px <- comp_data$open[i]
        current_pnl <- (exit_px / entry_px - 1) * 100
        ...
        log_message(sprintf("  [%s] TRADE EXIT at %.2f (PnL: %.2f%%) (next day open)", 
                           comp_data$date[i], exit_px, current_pnl))
      }
    }
...
has_trade_cols <- all(c("entry_price", "exit_price", "pnl_pct", "trade_active", "entry_date", "exit_date") %in% names(dt))
if (!isTRUE(params$recompute_trades) && isTRUE(has_trade_cols)) {
  log_message("Reusing trade columns from s3 output (use --recompute_trades to rebuild from status)")
  result_dt <- dt
} else {
  log_message("Recomputing trade columns from status signals (--recompute_trades enabled or trade columns missing)")
  ...
  result_dt <- process_trades_from_signals(dt, execute_next_day = params$execute_next_day)
}

# Save results only if recomputing trades (otherwise signal file is sufficient)
if (isTRUE(params$recompute_trades)) {
  if (!dir.exists("output/mmtm")) {
    dir.create("output/mmtm", recursive = TRUE)
  }
  output_file <- sprintf("output/mmtm/cleaned_trade_analysis_%s_%s.csv", params$scenario, format(params$ref_date, "%Y-%m-%d"))
  ...
  log_message("Skipping cleaned analysis file creation (signal file is sufficient when not recomputing trades)")
}
...
generate_trade_summary <- function(dt) {
  ...
  trades <- data.table(
    ...
    vol_category = character()  # ATR volatility category
  )
  ...
  for (comp_id in unique(dt$company_id)) {
    ...
    for (i in 1:nrow(comp_data)) {
      ...
      if (!in_trade && !is.na(current_row$entry_price) && current_row$entry_price > 0) {
        ...
        current_trade <- list(
          ...
          vol_category = current_row$vol_category  # Capture ATR volatility category at entry
        )
      }
      ...
      if (in_trade && !is.na(current_row$exit_price) && current_row$exit_price > 0) {
        ...
        trades <- rbind(trades, data.table(
          ...
          vol_category = current_trade$vol_category,  # Add ATR volatility category
          AbsolutePL_cum = 0,
          PercCumulativePL = 0,
    ...
    if (in_trade && !is.null(current_trade)) {
      ...
      trades <- rbind(trades, data.table(
        ...
        exit_trade_summary = "",
        exit_status = "",
        vol_category = current_trade$vol_category,  # Add ATR volatility category for open trades
        AbsolutePL_cum = 0,
        PercCumulativePL = 0,
...
if (nrow(trade_summary) > 0) {
  ...
  # Add ATR volatility analysis
  if (nrow(trade_summary) > 0 && "vol_category" %in% names(trade_summary)) {
    log_message("\n=== ATR VOLATILITY ANALYSIS ===")
    
    # Overall ATR volatility distribution
    atr_dist <- trade_summary[, .N, by = vol_category][order(vol_category)]
    log_message("Overall ATR Volatility Distribution:")
    for (i in 1:nrow(atr_dist)) {
      log_message(sprintf("  %s: %d trades (%.1f%%)", 
                         atr_dist$vol_category[i], 
                         atr_dist$N[i], 
                         atr_dist$N[i] / nrow(trade_summary) * 100))
    }
    
    # Performance by ATR volatility category
    atr_performance <- trade_summary[, {
      total_trades <- .N
      winning_trades <- sum(status == "WIN")
      losing_trades <- sum(status == "LOSS")
      win_rate <- if (total_trades > 0) round(winning_trades / total_trades * 100, 2) else 0
      avg_pnl <- if (total_trades > 0) round(mean(pnl_pct, na.rm = TRUE), 2) else 0
      avg_win <- if (winning_trades > 0) round(mean(pnl_pct[status == "WIN"], na.rm = TRUE), 2) else 0
      avg_loss <- if (losing_trades > 0) round(mean(pnl_pct[status == "LOSS"], na.rm = TRUE), 2) else 0
      profit_factor <- {
        total_profit <- sum(pnl_pct[status == "WIN"], na.rm = TRUE)
        total_loss <- abs(sum(pnl_pct[status == "LOSS"], na.rm = TRUE))
        if (total_loss > 0) round(total_profit / total_loss, 2) else Inf
      }
      ...
      .(total_trades = total_trades,
        ...
        avg_loss = avg_loss,
        profit_factor = profit_factor)
    }, by = .(company_id, company_name, vol_category)]
    
    # Log ATR volatility performance summary
    log_message("\nPerformance by ATR Volatility Category:")
    for (vol_cat in unique(atr_performance$vol_category)) {
      vol_data <- atr_performance[vol_category == vol_cat]
      if (nrow(vol_data) > 0) {
        log_message(sprintf("\n  %s Category:", vol_cat))
        log_message(sprintf("    Companies: %d", nrow(vol_data)))
        log_message(sprintf("    Total Trades: %d", sum(vol_data$total_trades)))
        log_message(sprintf("    Avg Win Rate: %.1f%%", mean(vol_data$win_rate)))
        log_message(sprintf("    Avg P&L: %.2f%%", mean(vol_data$avg_pnl)))
        log_message(sprintf("    Avg Profit Factor: %.2f", 
                       mean(sapply(vol_data$profit_factor, function(x) ifelse(is.infinite(x), NA, x)), na.rm = TRUE)))
      }
    }
    
    # Save ATR volatility performance to separate file
    atr_file <- file.path("output/mmtm", sprintf("atr_volatility_performance_%s_%s.csv", params$scenario, format(params$ref_date, "%Y-%m-%d")))
    fwrite(atr_performance, atr_file)
    log_message(sprintf("\nATR volatility performance saved to: %s", atr_file))
  }
  
  # Add fundamental analysis if companies data is available
  if (!is.null(companies_data) && nrow(companies_data) > 0 && nrow(trade_summary) > 0) {
    log_message("\n=== FUNDAMENTAL ANALYSIS ===")
    
    # Merge trade summary with companies data to add fundamental bucket columns
    trade_summary <- merge(
      trade_summary,
      companies_data[, .(company_id, mcap_bucket, sales_bucket, op_profit_bucket, pat_bucket)],
      by = "company_id",
      all.x = TRUE
    )
    
    log_message(sprintf("Added fundamental bucket columns to %d trades", nrow(trade_summary)))
    
    # Log fundamental performance summary (for logging purposes only)
    log_message("\nFundamental Analysis Summary:")
    log_message(sprintf("Market Cap Distribution: %s", paste(names(table(trade_summary$mcap_bucket, useNA = "ifany")), collapse = ", ")))
    log_message(sprintf("Sales Distribution: %s", paste(names(table(trade_summary$sales_bucket, useNA = "ifany")), collapse = ", ")))
    log_message(sprintf("Operating Profit Distribution: %s", paste(names(table(trade_summary$op_profit_bucket, useNA = "ifany")), collapse = ", ")))
    log_message(sprintf("PAT Distribution: %s", paste(names(table(trade_summary$pat_bucket, useNA = "ifany")), collapse = ", ")))
  }
  
  # Save both trade details and performance summary
  summary_dir <- "output/mmtm"
  
  # 1. Save detailed trades with company name
  trade_details_file <- file.path(summary_dir, sprintf("trade_details_%s_%s.csv", params$scenario, format(params$ref_date, "%Y-%m-%d")))
  ...
  metrics_file <- file.path(summary_dir, sprintf("performance_metrics_%s_%s.csv", params$scenario, format(params$ref_date, "%Y-%m-%d")))
...
if (!interactive()) {
  tryCatch({
    main()
    log_message("Trade tracker completed successfully", "SUCCESS")
  }, error = function(e) {
    ...
    log_message(sprintf("Fatal error in trade tracker: %s", error_msg), "ERROR")
