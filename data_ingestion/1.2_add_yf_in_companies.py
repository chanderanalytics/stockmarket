...
def fetch_and_update_yfinance_info(limit=None):
    ...
    try:
        ...
        print(f"Fetching yfinance info for {total} companies (smart comparison)" + (f" (limited to {limit})" if limit else "") + "...")
        logger.info(f"Fetching yfinance info for {total} companies (smart comparison)" + (f" (limited to {limit})" if limit else ""))
