"""
Daily Backup Corporate Actions Table Script

Simple backup script to create a daily backup of the corporate_actions table.
...
def backup_corporate_actions_table():
    """Create a daily backup of the corporate_actions table"""
    DB_NAME = 'stockdb'
    DB_USER = 'stockuser'
    DB_PASS = 'stockpass'
    DB_HOST = 'localhost'
    ...
    backup_table = f"corporate_actions_daily_{today}"
    ...
    cur.execute(f"""
        SELECT tablename 
        FROM pg_tables 
        WHERE tablename LIKE 'corporate_actions_daily_%' 
